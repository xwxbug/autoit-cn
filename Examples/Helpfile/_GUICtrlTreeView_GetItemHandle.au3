...
Func _Main()
	...
	MsgBox(4160, "Information", StringFormat( "Item handle
	for index %d: %s" ,  $iRand ,  _GUICtrlTreeView_GetItemHandle ( $hTreeView ,  $hItem [ $iRand ])))
		$iRand = Random(0, 29, 1)
		MsgBox(4160, "Information", StringFormat( "Item handle
		for child index %d: %s" ,  $iRand ,  _GUICtrlTreeView_GetItemHandle ( $hTreeView ,  $hItemChild [ $iRand ])))
			...
			GUIDelete()
;### Tidy Error -> "endfunc" is closing previous "for" on line 46
		endfunc   ;==>_Main
